...
class NintexToPowerAutomate:
    ...
    def extract_action_properties(self, action: ET.Element) -> Dict[str, Any]:
        """
        Extrae todas las propiedades de una acción de forma flexible
        """
        properties = {}
        try:
            ...
            # Extraer todos los elementos hijos como propiedades
            for child in action.iter():
                # Obtener el texto del elemento si existe
                if child.text and child.text.strip():
                    properties[child.tag] = child.text.strip()
                
                # Obtener todos los atributos
                for attr_name, attr_value in child.attrib.items():
                    properties[f"{child.tag}_{attr_name}"] = attr_value
            ...
            action_data = {
                ...
                'raw_content': ET.tostring(action, encoding='unicode'),  # Contenido completo
            ...
            return {
                'id': action.get('id', ''),
                'raw_content': ET.tostring(action, encoding='unicode'),  # Mantener contenido en caso de error
                'error': str(e)
            }
    ...
    def process_single_action(self, action_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print("Primer intento fallido, aplicando limpieza...")
                # Segundo intento: Limpiar y añadir comas
                json_str = re.sub(r'("\s+)"', '","', json_str)
                json_str = json_str.replace('\n', '').replace('\r', '')
                try:
                    power_automate_action = json.loads(json_str)
                except json.JSONDecodeError:
                    print("Error en la limpieza del JSON")
                    power_automate_action = {
                        "error": "JSON inválido después de limpieza",
            ...
            power_automate_action = {
                "error": f"No se pudo procesar la acción: {str(e)}",
                "original_action": action_data
            }
